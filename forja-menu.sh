...
screen_features() {
    ...
    case "$PERFIL" in
        minimal)
            FEATURE_NAMES=("opencode" "gemini" "sync-drive" "multiusuario")
            FEATURE_DESC=(
                "OpenCode -- Asistente IA TUI en la nube (OpenRouter)"
                "Gemini CLI -- Asistente IA Google (Gemini API)"
                "Sincronizacion Google Drive (rclone)"
                "Sistema multiusuario (alumnos)"
            )
            FEATURE_RAM=(50 50 50 50)
            FEATURE_DEFAULTS=(1 1 1 1)
            ;;
        moderado)
            # ESP32 disponible en arch y wsl, no en termux
            FEATURE_NAMES=("opencode" "gemini" "aider" "godot" "raylib" "n8n" "latex" "esp32" "sync-drive" "multiusuario")
            FEATURE_DESC=(
                "OpenCode -- Asistente IA TUI en la nube (OpenRouter)"
                "Gemini CLI -- Asistente IA Google (Gemini API)"
            ...
            FEATURE_RAM=(50 50 800 100 50 300 100 50 50 50)
            FEATURE_DEFAULTS=(1 1 1 1 1 1 1 0 1 1)
            ;;
        full)
            FEATURE_NAMES=("opencode" "gemini" "aider" "godot" "raylib" "unreal" "n8n" "picoclaw" "openclaw" "latex" "esp32" "fasm" "sync-drive" "multiusuario")
            FEATURE_DESC=(
                "OpenCode -- Asistente IA TUI en la nube (OpenRouter)"
                "Gemini CLI -- Asistente IA Google (Gemini API)"
                "Aider -- Asistente IA de codigo (Ollama)"
            ...
            FEATURE_RAM=(50 50 800 100 50 2000 300 20 1500 100 50 10 50 50)
            FEATURE_DEFAULTS=(1 1 1 1 1 0 1 1 1 1 1 1 1 1)
            ;;
    esac
